...
const ITEMS = [
  ...
  { id: "Cardamom Rice", name: "Cardamom Rice" },
  { id: "Cardamom Dust", name: "Cardamom Dust" },
  { id: "Other", name: "Other" }
];
...
export default function SaleForm() {
  ...
  const [itemName, setItemName] = useState("Cardamom");
  ...
  const submit = async () => {
    ...
    const aVal = parseFloat(amount);

    if (!branchId) { toast.show("Select branch", "error"); return; }
    if (!customerId) { toast.show("Select customer", "error"); return; }
    if (!itemName) { toast.show("Select item", "error"); return; }
    if (!qVal || qVal <= 0) { toast.show("Enter valid quantity", "error"); return; }
    if (isNaN(rVal) || rVal < 0) { toast.show("Enter valid rate", "error"); return; }
    if (isNaN(aVal) || aVal < 0) { toast.show("Enter valid amount", "error"); return; }
    ...
    try {
      ...
      toast.show("Sale transaction recorded");
      router.back();
    } catch (e: any) {
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
          <ScrollView contentContainerStyle={{ padding: spacing.xl, paddingBottom: 60 }} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.row}>
              <View style={{ flex: 1 }}>
                <Input testID="sale-quantity" label="Quantity (kg) *" keyboardType="decimal-pad" value={quantity} onChangeText={setQuantity} placeholder="0.00" />
              </View>
              <View style={{ flex: 1 }}>
                <Input testID="sale-rate" label="Rate (₹) *" keyboardType="decimal-pad" value={rate} onChangeText={setRate} placeholder="0.00" />
              </View>
            </View>

            <Input testID="sale-amount" label="Total Amount (₹) *" keyboardType="decimal-pad" value={amount} onChangeText={setAmount} placeholder="0.00" />
            ...
            <View style={styles.modeGrid}>
              {MODES.map(m => (
                <TouchableOpacity
                  key={m}
                  testID={`sale-mode-${m.toLowerCase()}`}
            ...
            <Input testID="sale-remarks" label="Remarks" value={remarks} onChangeText={setRemarks} multiline />

            <View style={{ gap: spacing.md, marginTop: spacing.lg }}>
              <Button testID="sale-submit" title="Record Sale" onPress={submit} loading={saving} />
            </View>
          </ScrollView>
